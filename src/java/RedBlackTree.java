...
import java.util.TreePrintUtil;
...
public class RedBlackTree {
	...
	//插入结点
	public void insertTreeNode(TreeNode tn){
		//临时引用
		TreeNode temp;
	...
	public void adjustment(){
		...
		if(current.getParent().getColor().equals(RED)){
			//且叔叔节点是红色
			TreeNode uncle = getUncleTreeNode(current);
	...
	//返回当前指针
	public TreeNode searchActive(){
		return active;
		
	}
	
	//获得叔叔结点
	public TreeNode getUncleTreeNode(TreeNode tn){
	...
	public void locationOfJudgment(TreeNode me){
		TreeNode parent=me.getParent();
		TreeNode grandpa=parent.getParent();
		
		TreeNode leftUncle=grandpa.getLeft();
		TreeNode rightUncle=grandpa.getRight();
		
		TreeNode leftBro=parent.getLeft();
		//我在父亲的左侧
	...
	public void delete(int value){
		try {
			TreeNode max=findMaximinAndCopyValue(findNode(value));
			//调用方法
			deleteNode(max);
		} catch (Exception e) {
	...
	//找到要删除的结点
	public TreeNode findNode(int value) throws Exception{
		TreeNode temp=root;
	...
	public TreeNode findMaximinAndCopyValue(TreeNode current){
		//保存当前指针的引用
	...
	public TreeNode getSub(TreeNode current){
		return current.getLeft();
	}
	
	//得到兄弟结点
	public TreeNode getBro(TreeNode current){
	...
	//左旋
	public void leftRotale(TreeNode tn){
		//获得三个需要改变的结点，加上参数这个支点，一共四个
		TreeNode grandpa=tn.getParent();
		TreeNode rightSon=tn.getRight();
		TreeNode leftgrandson=rightSon.getLeft();
		//开始左旋
		//右孩子变为祖父的孩子
		if(grandpa != null){
	...
	//右旋
	public void rightRotale(TreeNode tn){
		//获得三个需要改变的结点，加上参数这个支点，一共四个
		TreeNode grandpa=tn.getParent();
		TreeNode leftSon=tn.getLeft();
		TreeNode rightgrandson=leftSon.getRight();
		//开始右旋
		//左孩子变为祖父的孩子
		if(grandpa != null){
			if(grandpa.getLeft() != null && grandpa.getLeft().equals(tn)){
